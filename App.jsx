...
// Billomat API fetch with pagination
async function fetchAllInvoices(billomatId, apiKey, setStatus) {
  let page = 1
  let all = []
  const perPage = 100

  while (true) {
    setStatus(`Lade Seite ${page}…`)
    const url = `https://${billomatId}.billomat.net/api/invoices?per_page=${perPage}&page=${page}`
    ...
    all = [...all, ...arr]
    const total = parseInt(data?.invoices?.['@total'] || '0', 10)
...
export default function App() {
  const [billomatId, setBillomatId] = useState(() => localStorage.getItem('bm_id') || '')
  const [apiKey, setApiKey] = useState(() => localStorage.getItem('bm_key') || '')
  ...
  const load = useCallback(async () => {
    if (!billomatId || !apiKey) return
    setLoading(true)
    setError('')
    setStatus('Verbinde mit Billomat…')
    try {
      localStorage.setItem('bm_id', billomatId)
      localStorage.setItem('bm_key', apiKey)
      const data = await fetchAllInvoices(billomatId, apiKey, setStatus)
      setInvoices(data)
        ...
        {!configured && (
          <div style={{
            ...
            <div style={{ color: 'var(--text-muted)', fontSize: 13, marginBottom: 28 }}>
              Gib deine Billomat-ID und den API-Schlüssel ein, um die Rechnungsauswertung zu starten.
            </div>
            ...
            <label style={{ display: 'block', marginBottom: 16 }}>
              ...
              <input
                value={billomatId}
                onChange={e => setBillomatId(e.target.value.trim())}
              ...
              <div style={{ color: 'var(--text-muted)', fontSize: 11, marginTop: 4 }}>
                → https://<strong style={{ color: 'var(--accent)' }}>meinefirma</strong>.billomat.net
              </div>
            </label>
            ...
            <label style={{ display: 'block', marginBottom: 24 }}>
              ...
              />
              <div style={{ color: 'var(--text-muted)', fontSize: 11, marginTop: 4 }}>
                Billomat → Benutzereinstellungen → API-Schlüssel
              </div>
            </label>
            ...
            <button
              onClick={load}
              disabled={!billomatId || !apiKey || loading}
              style={{
                width: '100%', padding: '14px', borderRadius: 10,
                border: 'none', background: 'var(--accent)', color: '#0a0a0f',
                fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 15,
                opacity: (!billomatId || !apiKey || loading) ? 0.5 : 1,
            ...
            <div style={{
              ...
              🔒 Deine Zugangsdaten werden nur lokal im Browser gespeichert und niemals übertragen.
